...
class Payments extends admin 
{
	...
	*/
	public function index($order = 'mpesalipatransaction.mpesalipatransaction_id', $order_method = 'DESC') 
	{
		...
		$config['base_url'] = site_url().'reports/payments/'.$order.'/'.$order_method;
		...
		$v_data['transaction_codes'] = $this->payments_model->get_transaction_codes();
	...
	{
		$this->payments_model->update_payment_info();
	}
    
	public function test_page()
	{
		$v_data['transacted_agents'] = $this->payments_model->get_transacted_agents();
		$this->load->view("payments/test.php", $v_data);
	}
	...
	public function search_payments()
	{
		...
		{
			$search_title .= ' Transaction Code <strong>'.$transaction_code.'</strong>';
			$transaction_code = ' AND mpesalipatransaction.mpesaReceiptNumber = \''.$transaction_code.'\'';
		}
		...
		if(!empty($result_code))
		{
			if($result_code == "zero")
			{
				$search_title .= ' Result Code <strong>0</strong>';
				$result_code = ' AND mpesalipatransaction.resultCode = \'0\'';
			}
		...
		$search = $customer_phone.$daterange.$payment_location.$result_code.$transaction_code;
	...
	{
		$order = 'mpesalipatransaction.resultReceived';
		$order_method = 'DESC';
		$where = 'mpesalipatransaction.resultCode = 0';
		$table = 'mpesalipatransaction';
		$payments_search = $this->session->userdata('payments_search');
		$search_title = $this->session->userdata('payments_search_title');
		...
		if(!empty($search_title) && $search_title != NULL)
		{
			$title = 'Payments filtered by '.$search_title;
		}
		...
		if ($this->form_validation->run())
		{
			...
			if($total_payments > 0)
			{
				foreach($mpesalipatransaction_id_array as $key => $value)
				{
					...
					{
						$this->payments_model->delete_payment($value);
					}
					else
					{
